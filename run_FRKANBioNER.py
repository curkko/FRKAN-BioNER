...
from FRKANBioNER import FRKANBioNER
...
def main():
    ...
    parser.add_argument("--use_kan",
                        action='store_true',
                        help="Set this flag if you are using kan.")
    ...
    tokenizer = AutoTokenizer.from_pretrained("./biobert_diseases_ner", do_lower_case=args.do_lower_case)
    ...
    logger.info(args)
    # Prepare model
    model = FRKANBioNER(args,
                  hidden_dropout_prob=args.hidden_dropout_prob,
                  num_labels=num_labels,
                  windows_list=[int(k) for k in
                                args.windows_list.split('qq')] if args.windows_list else args.window_size,
                  )
    ...
    if args.do_train:
            ...
            for input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, seq_len in eval_dataloader:
                ...
                with torch.no_grad():
                    logits = model(input_ids, segment_ids, input_mask, valid_ids=valid_ids,
                                   attention_mask_label=l_mask)  # , seq_len=seq_len)

                if not args.use_crf:
                    shape = logits.shape
                    if len(shape) == 2:  # (batch_size, sequence_length)
                        logits = torch.argmax(F.log_softmax(logits, dim=1), dim=1)
                    elif len(shape) == 3:  # (batch_size, sequence_length, num_classes)
        ...
        torch.save(model, 'saved_model_path/model.pt')
    ...
    if args.do_predict:
        ...
        test_examples = processor.get_test_examples(args.test_data_dir)
        test_features, ori_sents = convert_examples_to_features(test_examples, label_list, args.max_seq_length,
                                                                tokenizer)
        all_input_ids_dev = torch.tensor([f.input_ids for f in test_features], dtype=torch.long)
        all_input_mask_dev = torch.tensor([f.input_mask for f in test_features], dtype=torch.long)
        all_segment_ids_dev = torch.tensor([f.segment_ids for f in test_features], dtype=torch.long)
        ...
        with open(output_result_file, "w", encoding='utf-8') as writer:
